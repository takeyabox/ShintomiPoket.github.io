...
入室中に通信が切れた場合はルーム購読を一時停止し、通信復旧後に「部屋に復帰する」または「復帰せず部屋を出る」をプレイヤーが選びます。ページを閉じて後からログインした場合も、保存済みの部屋が残っていれば同じ確認画面を表示し、自動では復帰しません。退出を選ぶとFirebaseから自分の参加情報を削除し、残った相手側の部屋を待機状態へ戻します。
